...
class MyRecommender(BaseRecommender):
    """
    Custom recommender based on a scikit-learn compatible Logistic Regression Model.
...
param_ranges = {
    ...
    'C': (0.01, 10.0, 0.5),  # inverse regularization strength
...
current_params = {param: random.choice(choices) for param, choices in param_ranges.items()}
best_revenue = -np.inf
...
while improvement >= tolerance and iter < 50:  # Limit iterations to avoid infinite loop
    ...
    improvement = 0
    for param, choices in param_ranges.items():
        ...
        for value in choices:
            ...
            recommender = LogRegModel(seed = 42, **trial_params)
            recommender.fit(
...
df_results.to_csv('logistic_regression_adaptive_search_results.csv', index=False)

print("\nBest parameters found:")
print(current_params)
print(f"Best test revenue: ${best_revenue:.2f}")
print("Results saved to 'logistic_regression_adaptive_search_results.csv'")
